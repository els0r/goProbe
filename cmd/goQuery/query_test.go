...
var emptyOutputArgs = [][]string{
	{"-i", "eth1", "-d", SmallGoDB, "-f", "-30000d", "-c", "dport < 100 & dport > 100", "-e", "json", "talk_conv"},
	// border case:
	...
	{"-i", "eth1", "-d", SmallGoDB, "-f", "-30000d", "-l", "1456428575", "-e", "json", "raw"},
}
...
func TestEmptyOutput(t *testing.T) {
	...
	for _, args := range emptyOutputArgs {
		...
		if actualOutput["status"] != "empty" || actualOutput["statusMessage"] != errorNoResults {
...
var dnsArgs = []string{
	"-i", "eth1", "-d", SmallGoDB, "-f", "-30000d", "-c", "sip = 1b5ec0b902e689122ededcfac01ee69ed3c8422c.open.ch", "talk_conv",
}
