...
export function AttributesSelect({ options, value, allSelected, onChange, hasError }: AttributesSelectProps) {
    ...
    <div className="relative text-sm w-full" ref={ref}>
      <button
        ...
        className={
          ...
          (hasError ? 'ring-red-500/40 bg-red-500/10 focus:outline-none focus:ring-red-500/40' : 'ring-white/10 focus:outline-none focus:ring-primary-500')
        }
      >
        ...
        <div className="absolute z-20 mt-1 w-full rounded-md border border-white/10 bg-surface-100 p-1 shadow-lg">
          <ul className="max-h-60 overflow-auto scroll-thin text-xs">
            {options.map(o => {
              ...
              const checked = allSelected ? true : value.some(v => (ATTR_ALIASES[v] || v) === canonical)
              return (
                <li key={o.value}>
...
const ATTR_ALIASES: Record<string,string> = { protocol: 'proto', port: 'dport' }

export function parseAttributeQuery(q: string | undefined): { values: string[]; all: boolean } {
