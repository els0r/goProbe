...
export function HostDetailsPanel({ host, rows, loading, error, summary, onClose }: HostDetailsPanelProps) {
  // close on Escape
  useEffect(() => {
  ...
  const byIface = (() => {
    const m = new Map<string, { iface: string; inB: number; outB: number; inP: number; outP: number }>()
    for (const r of rows) {
  ...
  return (
    <div className="absolute right-2 top-2 z-20 w-[360px] max-h-[66vh] overflow-hidden">
      <DetailsPanel title={host} totalBytes={totals.inB + totals.outB} totalPackets={totals.inP + totals.outP} onClose={onClose}>
        {error ? (
        ...
        <div className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-gray-300">Interfaces</div>
        {summary && (
