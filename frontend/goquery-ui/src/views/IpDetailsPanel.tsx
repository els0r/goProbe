...
export function IpDetailsPanel({ ip, rows, loading, error, onClose, summary }: IpDetailsPanelProps) {
  // close on Escape
  useEffect(() => {
  ...
  return (
    <div className="absolute right-2 top-2 z-20 w-[360px] max-h-[66vh] overflow-hidden">
      <DetailsPanel title={ip} totalBytes={totals.inB + totals.outB} totalPackets={totals.inP + totals.outP} onClose={onClose}>
        {error ? (
