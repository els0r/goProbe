...
function formatInOffset(ms: number, offsetMin: number, includeDate: boolean, includeSuffix: boolean, suffix: string): string {
...
function buildIntervalLabel(endIso: string, durMs: number, prevEndIso?: string): { label: string; isNewDate: boolean } {
  ...
  const prevDay = prevEndIso && Number.isFinite(Date.parse(prevEndIso))
...
export function TemporalDetailsPanel({ meta, attrsShown, rows, loading, error, summary, onClose }: TemporalDetailsPanelProps) {
  // close on Escape
  useEffect(() => {
  ...
  const durMs: number = Number.isFinite(resolutionNs) ? Math.max(1, Math.round(Number(resolutionNs) / 1_000_000)) : 5 * 60 * 1000
  ...
  const lastMs = tfLast ? Date.parse(tfLast) : (times.length ? Math.max(...times.map(t => Date.parse(t))) : Date.now())
  const firstMs = tfFirst ? Date.parse(tfFirst) : (times.length ? Math.min(...times.map(t => Date.parse(t))) : lastMs - 12 * 60 * 60 * 1000)
  ...
  let tileMs = niceDurationsMs.find(d => d >= requiredPerTile) || niceDurationsMs[niceDurationsMs.length - 1]
  ...
  // initialize buckets
  const buckets: Array<{ start: number; end: number; bytes: number; firstRowIndex?: number }> = Array.from({ length: gridTileCount }, (_, i) => {
  ...
  return (
    <div className="absolute right-2 top-2 z-20 w-[680px] max-h-[70vh]">
      <DetailsPanel
        title={<span title={meta.host}>{meta.host} — {meta.iface}</span>}
      ...
      >
        ...
        <div className="mb-3 rounded-md bg-surface-200/40 px-3 py-2 text-[12px] ring-1 ring-white/10">
          <div className="flex flex-wrap items-baseline gap-x-6 gap-y-1">
            {attrsShown.includes('sip') && (
              <div><span className="mr-1 text-gray-400">SIP</span><span className="font-mono break-all">{meta.sip}</span></div>
            )}
            {attrsShown.includes('dip') && (
              <div><span className="mr-1 text-gray-400">DIP</span><span className="font-mono break-all">{meta.dip}</span></div>
            )}
            {attrsShown.includes('dport') && meta.dport !== undefined && meta.dport !== null && (
              <div><span className="mr-1 text-gray-400">DPORT</span><span className="font-mono">{meta.dport}</span></div>
            )}
            {attrsShown.includes('proto') && meta.proto !== undefined && meta.proto !== null && (
              <div><span className="mr-1 text-gray-400">PROTO</span><span className="font-mono">{renderProto(meta.proto)}</span></div>
            )}
          </div>
        </div>
        {error ? (
        ...
        <div className="mb-2 relative">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-10 select-none">
            <div className=
              "grid h-full grid-rows-6 content-between pr-1 text-right text-[10px] text-gray-400">
              {Array.from({ length: 6 }).map((_, i) => {
                let label = ''
                if (i === 2) label = formatShortDuration(3 * tileMs)
                if (i === 5) label = formatShortDuration(6 * tileMs)
                return (
                  <div key={i} className="flex items-center justify-end">{label}</div>
                )
              })}
            </div>
          </div>
          ...
          <div className="pl-10">
            <div className="grid gap-[3px] sm:gap-1" style={{ gridTemplateColumns: 'repeat(24, minmax(0, 1fr))' }}>
              {buckets.map((b, i) => {
        ...
        {(() => {
          ...
          const addLabel = (hours: number) => {
            ...
            if (!labels.some(l => Math.abs(l.col - col) < 2)) labels.push({ col, text: fmtAxis(hours) })
          }
          ...
          const cells = Array.from({ length: 24 }, (_, i) => labels.find(l => l.col === i)?.text || '')
          return (
            <div className="mb-2 select-none text-[10px] text-gray-400 pl-10">
              <div className="grid" style={{ gridTemplateColumns: 'repeat(24, minmax(0, 1fr))' }}>
                {cells.map((txt, i) => (
                  <div key={i} className="text-center">{txt}</div>
                ))}
              </div>
            </div>
          )
        })()}
        <div className="mb-1 text-[12px] text-gray-300">Found in <span className="font-semibold text-white">{uniqueTimes.length}</span> time intervals</div>
        <div ref={listRef} className="scroll-thin max-h-[50vh] overflow-auto pr-1">
          ...
          {!loading && rows.map((r, i) => {
            ...
            const uni = ((r.bytes_in || 0) + (r.packets_in || 0) === 0) || ((r.bytes_out || 0) + (r.packets_out || 0) === 0)
            const baseClass = isNewDate ? 'bg-surface-200/60 border-white/10' : 'bg-surface-100/60 border-white/10'
            const backgroundClass = uni ? 'bg-red-400/15 ring-1 ring-red-400/20 hover:bg-red-400/25' : baseClass
            return (
              <div key={i} ref={el => (rowRefs.current[i] = el)} className={`mb-2 ${highlightIndex === i ? 'ring-2 ring-primary-500 rounded-xl' : ''}`} data-unidirectional={uni || undefined}>
                <DetailsCard
                  heading={label}
                  totalBytes={totalB}
                  totalPackets={totalP}
