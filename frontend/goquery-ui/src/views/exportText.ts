...
export function buildTextTable(rows: FlowRecord[], opts: BuildTextOptions = {}): string {
  ...
  const headerLine1Padding = cols.slice(0, firstMetricIdx).reduce((s, c) => s + c.width + spacerBetween.length, 0)
  const header1 = ' '.repeat(Math.max(0, headerLine1Padding)) +
    pad('packets', cols[firstMetricIdx].width, 'right') + spacerBetween +
    pad('packets', cols[firstMetricIdx + 1].width, 'right') + spacerBetween +
    pad('', cols[firstMetricIdx + 2].width, 'right') + spacerBetween +
    pad('bytes', cols[firstMetricIdx + 3].width, 'right') + spacerBetween +
    pad('bytes', cols[firstMetricIdx + 4].width, 'right')

  const header2Labels: Record<string, string> = {
    host: 'host', iface: 'iface', sip: 'sip', dip: 'dip', dport: 'dport', proto: 'proto',
    pin: 'in', pout: 'out', ppct: '%', bin: 'in', bout: 'out', bpct: '%',
  }
  const header2 = cols.map(c => pad(header2Labels[c.key] || c.key, c.width, c.align)).join(spacerBetween)
  ...
  const totalHits = Number.isFinite(Number(opts?.meta?.hitsTotal)) ? Number(opts?.meta?.hitsTotal) : undefined
  if (totalHits !== undefined && rows.length < totalHits) {
    ...
    const ellLine = cols.map(c => pad((ellVals as any)[c.key] || '', c.width, c.align)).join(spacerBetween)
    lines.push('  ' + ellLine)
  ...
  pout: humanPackets(Math.max(0, Number(opts?.meta?.ps) || 0)),
    ...
    const totLine = cols.map(c => pad((tVals as any)[c.key] || '', c.width, c.align)).join(spacerBetween)
    lines.push('  ' + totLine)
  }
  ...
  lines.push('')
  {
    ...
    const bottomLine = cols.map(c => pad((bottomVals as any)[c.key] || '', c.width, c.align)).join(spacerBetween)
  ...
  const rangeTxt = (() => {
    ...
    const s = Math.floor(ms / secMs); ms -= s * secMs
    const parts: string[] = []
  ...
  lines.push('Sorted by          : ' + (opts?.meta?.sortBy === 'packets' ? 'packet count (sent and received)' : 'accumulated data volume (sent and received)'))
  const hits = totalHits !== undefined ? ` out of ${totalHits}` : ''
  lines.push('Query stats        : displayed top ' + rows.length + ' hits' + hits + (durTxt ? ' in ' + durTxt : ''))
